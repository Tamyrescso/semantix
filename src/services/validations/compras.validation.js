...
const validateOrder = (clienteId, compras, clienteOuCompra) => {
  ...
  if (errorQuantity.some((e) => e)) return errorQuantity.find((e) => e);
...
const ifClientExist = async (clienteId) => {
  ...
  if (!clientExists) return { code: 404, data: { message: "Client doesn't exist" } };
...
const ifOrderExist = async (compraId) => {
  ...
  if (!orderExists) return { code: 404, data: { message: "Order doesn't exist" } };
...
const ifProductsExist = async (compras) => {
  ...
  if(productNotExist.some((product) => !product)) return { code: 404, data: { message: "Product doesn't exist" } }
}
